...
class PyCA_Cryptography_SHA512_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA512_Test(std::string input_message,
  ...
  auto TestBody() -> void override {
    std::ostringstream result;
    sourcemeta::core::sha512(this->message, result);
...
class PyCA_Cryptography_SHA512_Monte_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA512_Monte_Test(
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Len = ")) {
      ...
      testing::RegisterTest(
          ...
          __LINE__, [=]() -> PyCA_Cryptography_SHA512_Test * {
            return new PyCA_Cryptography_SHA512_Test(message, expected_digest);
          });
    }
  }
}
...
static auto register_monte_tests(const std::filesystem::path &file_path)
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Seed = ")) {
      ...
      testing::RegisterTest("PyCA_Cryptography_SHA512_Monte", test_name.c_str(),
                            nullptr, nullptr, __FILE__, __LINE__,
                            [=]() -> PyCA_Cryptography_SHA512_Monte_Test * {
                              return new PyCA_Cryptography_SHA512_Monte_Test(
...
auto main(int argc, char **argv) -> int {
  ...
  register_msg_tests(sha2_path / "SHA512ShortMsg.rsp",
                     "PyCA_Cryptography_SHA512_ShortMsg");
  register_msg_tests(sha2_path / "SHA512LongMsg.rsp",
                     "PyCA_Cryptography_SHA512_LongMsg");
  register_monte_tests(sha2_path / "SHA512Monte.rsp");
