...
class PyCA_Cryptography_SHA384_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA384_Test(std::string input_message,
  ...
  auto TestBody() -> void override {
    std::ostringstream result;
    sourcemeta::core::sha384(this->message, result);
...
class PyCA_Cryptography_SHA384_Monte_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA384_Monte_Test(
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Len = ")) {
      ...
      testing::RegisterTest(
          ...
          __LINE__, [=]() -> PyCA_Cryptography_SHA384_Test * {
            return new PyCA_Cryptography_SHA384_Test(message, expected_digest);
          });
    }
  }
}
...
static auto register_monte_tests(const std::filesystem::path &file_path)
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Seed = ")) {
      ...
      testing::RegisterTest("PyCA_Cryptography_SHA384_Monte", test_name.c_str(),
                            nullptr, nullptr, __FILE__, __LINE__,
                            [=]() -> PyCA_Cryptography_SHA384_Monte_Test * {
                              return new PyCA_Cryptography_SHA384_Monte_Test(
...
auto main(int argc, char **argv) -> int {
  ...
  register_msg_tests(sha2_path / "SHA384ShortMsg.rsp",
                     "PyCA_Cryptography_SHA384_ShortMsg");
  register_msg_tests(sha2_path / "SHA384LongMsg.rsp",
                     "PyCA_Cryptography_SHA384_LongMsg");
  register_monte_tests(sha2_path / "SHA384Monte.rsp");
