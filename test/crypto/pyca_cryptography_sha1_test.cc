...
class PyCA_Cryptography_SHA1_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA1_Test(std::string input_message,
  ...
  auto TestBody() -> void override {
    std::ostringstream result;
    sourcemeta::core::sha1(this->message, result);
...
class PyCA_Cryptography_SHA1_Monte_Test : public testing::Test {
public:
  explicit PyCA_Cryptography_SHA1_Monte_Test(std::string input_seed,
                                             std::string input_expected_digest)
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Len = ")) {
      ...
      testing::RegisterTest(
          ...
          __LINE__, [=]() -> PyCA_Cryptography_SHA1_Test * {
            return new PyCA_Cryptography_SHA1_Test(message, expected_digest);
          });
    }
  }
}
...
static auto register_monte_tests(const std::filesystem::path &file_path)
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.starts_with("Seed = ")) {
      ...
      testing::RegisterTest("PyCA_Cryptography_SHA1_Monte", test_name.c_str(),
                            nullptr, nullptr, __FILE__, __LINE__,
                            [=]() -> PyCA_Cryptography_SHA1_Monte_Test * {
                              return new PyCA_Cryptography_SHA1_Monte_Test(
...
auto main(int argc, char **argv) -> int {
  ...
  const auto sha1_path = suite_path / "hashes" / "SHA1";

  register_msg_tests(sha1_path / "SHA1ShortMsg.rsp",
                     "PyCA_Cryptography_SHA1_ShortMsg");
  register_msg_tests(sha1_path / "SHA1LongMsg.rsp",
                     "PyCA_Cryptography_SHA1_LongMsg");
  register_monte_tests(sha1_path / "SHA1Monte.rsp");
