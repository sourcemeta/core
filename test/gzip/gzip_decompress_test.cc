...
// RFC 1951 section 3.2.5 assigns lengths 227 to 257 to length code 284, but
// like zlib the decoder accepts its extra bits reaching 258
TEST(decompress_fixed_block_length_258_from_code_284) {
  const std::vector<std::uint8_t> input{
      0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x4b, 0x1c,
      0xf9, 0x00, 0x00, 0x56, 0xfa, 0xc2, 0x34, 0x03, 0x01, 0x00, 0x00};
...
TEST(decompress_second_member_fixed_header_spanning_stream_source_buffer) {
  std::vector<std::uint8_t> input{0x1f, 0x8b, 0x08, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0xff,
                                  0x01, 0xe7, 0xff, 0x18, 0x00};
  input.insert(input.end(), 65511, 0x00);
  input.insert(input.end(),
               {0xef, 0x77, 0x5e, 0xcb, 0xe7, 0xff, 0x00, 0x00, 0x1f,
                0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
                0x01, 0x0b, 0x00, 0xf4, 0xff, 0x68, 0x65, 0x6c, 0x6c,
                0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x85, 0x11,
                0x4a, 0x0d, 0x0b, 0x00, 0x00, 0x00});
  EXPECT_GZIP_DECOMPRESS(input, std::string(65511, '\0') + "hello world");
}

TEST(decompress_second_member_header_checksum_spanning_stream_source_buffer) {
  std::vector<std::uint8_t> input{0x1f, 0x8b, 0x08, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0xff,
                                  0x01, 0xde, 0xff, 0x21, 0x00};
  input.insert(input.end(), 65502, 0x00);
  input.insert(input.end(),
               {0x91, 0x06, 0x5e, 0x62, 0xde, 0xff, 0x00, 0x00, 0x1f,
                0x8b, 0x08, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
                0x90, 0xc9, 0x01, 0x0b, 0x00, 0xf4, 0xff, 0x68, 0x65,
                0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64,
                0x85, 0x11, 0x4a, 0x0d, 0x0b, 0x00, 0x00, 0x00});
  EXPECT_GZIP_DECOMPRESS(input, std::string(65502, '\0') + "hello world");
}

TEST(decompress_second_member_extra_length_spanning_stream_source_buffer) {
  std::vector<std::uint8_t> input{0x1f, 0x8b, 0x08, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0xff,
                                  0x01, 0xde, 0xff, 0x21, 0x00};
  input.insert(input.end(), 65502, 0x00);
  input.insert(input.end(),
               {0x91, 0x06, 0x5e, 0x62, 0xde, 0xff, 0x00, 0x00, 0x1f,
                0x8b, 0x08, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
                0x00, 0x00, 0x01, 0x0b, 0x00, 0xf4, 0xff, 0x68, 0x65,
                0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64,
                0x85, 0x11, 0x4a, 0x0d, 0x0b, 0x00, 0x00, 0x00});
  EXPECT_GZIP_DECOMPRESS(input, std::string(65502, '\0') + "hello world");
}
