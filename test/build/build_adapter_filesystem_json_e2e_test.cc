...
auto HANDLER_MULTIPLY(
    ...
    const sourcemeta::core::BuildDependencies<typename Adapter::node_type>
        &dependencies,
    const sourcemeta::core::BuildDynamicCallback<typename Adapter::node_type> &,
...
TEST(Build_Adapter_Filesystem_JSON_e2e, simple_cache_miss_hit) {
  ...
  const auto result_2 = sourcemeta::core::build<Context>(
  ...
  const auto result_3 = sourcemeta::core::build<Context>(
      adapter, HANDLER_MULTIPLY<Context, Adapter>, base_path / "first.txt",
      {base_path / "initial.txt"}, 2);
  ...
  const auto result_4 = sourcemeta::core::build<Context>(
      adapter, HANDLER_MULTIPLY<Context, Adapter>, base_path / "second.txt",
      {base_path / "first.txt"}, 3);
  ...
  // Create second.txt from first.txt (cache miss)
  const auto result_6 = sourcemeta::core::build<Context>(
      adapter, HANDLER_MULTIPLY<Context, Adapter>, base_path / "second.txt",
      {base_path / "first.txt"}, 3);
