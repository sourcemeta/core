...
TEST(valid_length_measured_on_mapped_form) {
  ...
  static_assert(INPUT.size() == 189, "literal length");
...
TEST(invalid_length_measured_on_mapped_form_at_64) {
  ...
  static_assert(INPUT.size() == 192, "literal length");
...
TEST(valid_ignored_codepoint_not_counted_in_length) {
  ...
  static_assert(INPUT.size() == 66, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_ignored_codepoint_label_still_over) {
  ...
  static_assert(INPUT.size() == 67, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_ace_prefix_produced_by_mapping) {
  static constexpr std::string_view INPUT{"\xef\xbd\x98\xef\xbd\x8e--nxasmq6b"};
  static_assert(INPUT.size() == 16, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_ace_prefix_produced_by_mapping_bad_payload) {
  static constexpr std::string_view INPUT{"\xef\xbd\x98\xef\xbd\x8e--b"};
  static_assert(INPUT.size() == 9, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_hyphen_produced_by_mapping_leading) {
  static constexpr std::string_view INPUT{"\xef\xbc\x8d"
                                          "abc"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_hyphen_produced_by_mapping_trailing) {
  static constexpr std::string_view INPUT{"abc\xef\xbc\x8d"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_double_hyphen_produced_by_mapping) {
  static constexpr std::string_view INPUT{"ab\xef\xbc\x8d\xef\xbc\x8d"
                                          "cd"};
  static_assert(INPUT.size() == 10, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_empty_label_produced_by_mapping) {
  static constexpr std::string_view INPUT{"a\xef\xbc\x8e\xef\xbc\x8e"
                                          "b"};
  static_assert(INPUT.size() == 8, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_deviation_final_sigma_kept) {
  static constexpr std::string_view INPUT{"\xcf\x82"};
  static_assert(INPUT.size() == 2, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
...
TEST(invalid_deviation_zwj_without_context) {
  static constexpr std::string_view INPUT{"a\xe2\x80\x8d"
                                          "b"};
  static_assert(INPUT.size() == 5, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_mathematical_bold_is_mapped) {
  static constexpr std::string_view INPUT{"\xf0\x9d\x90\x80"
                                          "bc"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
...
TEST(valid_variation_selector_is_ignored) {
  static constexpr std::string_view INPUT{"a\xef\xb8\x80"
                                          "b"};
  static_assert(INPUT.size() == 5, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
...
TEST(valid_circled_latin_is_mapped) {
  static constexpr std::string_view INPUT{"\xe2\x93\x90"
                                          "bc"};
  static_assert(INPUT.size() == 5, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
...
TEST(valid_ligature_is_mapped) {
  static constexpr std::string_view INPUT{"\xef\xac\x81x"};
  static_assert(INPUT.size() == 4, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_underscore_nv8) {
  static constexpr std::string_view INPUT{"a_b"};
  static_assert(INPUT.size() == 3, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_nfc_composition_by_mapping) {
  static constexpr std::string_view INPUT{"cafe\xcc\x81"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_bidi_ltr_char_inside_rtl_label) {
  static constexpr std::string_view INPUT{"\xd7\x90"
                                          "a\xd7\x90"};
  static_assert(INPUT.size() == 5, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
...
TEST(invalid_bidi_rtl_label_ends_with_on) {
  static constexpr std::string_view INPUT{"\xd7\x90\xcb\x87"};
  static_assert(INPUT.size() == 4, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_bidi_pure_arabic_indic_digit_label) {
  static constexpr std::string_view INPUT{"\xd9\xa0\xd9\xa1"};
  static_assert(INPUT.size() == 4, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_extended_arabic_indic_digits_only) {
  static constexpr std::string_view INPUT{"\xd8\xa8\xdb\xb0\xdb\xb1"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(invalid_extended_arabic_indic_mixed_with_arabic_indic) {
  static constexpr std::string_view INPUT{"\xd8\xa8\xdb\xb0\xd9\xa0"};
  static_assert(INPUT.size() == 6, "literal length");
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_FALSE(sourcemeta::core::is_idn_hostname(INPUT));
}
...
TEST(valid_arabic_indic_digit_blocks_in_separate_labels) {
  static constexpr std::string_view INPUT{"\xd8\xa8\xd9\xa0.\xd8\xa8\xdb\xb0"};
  static_assert(INPUT.size() == 9, "literal length");
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname_uts46(INPUT));
  EXPECT_TRUE(sourcemeta::core::is_idn_hostname(INPUT));
}
