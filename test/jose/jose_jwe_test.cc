...
TEST(from_parses_an_empty_encrypted_key) {
  // The direct algorithms carry an empty Encrypted Key segment (RFC 7516
  // Section 7.1)
  std::string compact{
  ...
  EXPECT_TRUE(object.has_value());
  EXPECT_TRUE(object.value().encrypted_key().empty());
}
...
TEST(from_rejects_a_compression_header) {
  EXPECT_FALSE(
      sourcemeta::core::JWE::from(
          compact_from_header(R"({"alg":"dir","enc":"A128GCM","zip":"DEF"})"))
          .has_value());
}
