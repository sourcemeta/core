...
// Check if this is an error test (expected_obj: "error")
static auto is_error_test(const sourcemeta::core::JSON &test_case) -> bool {
  return test_case.defines("expected_obj") &&
         test_case.at("expected_obj").is_string() &&
         test_case.at("expected_obj").to_string() == "error";
}

// TODO: Remove this filter and handle all tests
static auto do_we_handle_match_test(const sourcemeta::core::JSON &test_case)
    -> bool {
  // Handle error tests (pattern parsing should fail)
  if (is_error_test(test_case)) {
    if (!test_case.defines("pattern")) {
      return false;
    }
    ...
    // Only handle object patterns for now
    ...
    // Skip error tests that require validations not yet implemented:
    // - Emoji/Unicode identifier validation (requires full ECMAScript ID check)
    // - IPv6 pattern validation
    // - Duplicate named group detection
    // - Hostname character validation
    for (const auto &entry : pattern_item.as_object()) {
      const auto &value = entry.second.to_string();

      // Skip patterns with IPv6 addresses or brackets (complex IPv6 validation)
      if (value.find('[') != std::string::npos ||
          value.find(']') != std::string::npos) {
        return false;
      }

      // Skip pathname patterns with duplicate named groups (:id/:id)
      if (entry.first == "pathname" && value.find("/:") != std::string::npos) {
        auto pos = value.find("/:");
        while (pos != std::string::npos) {
          auto name_end = pos + 2;
          while (name_end < value.size() &&
                 (std::isalnum(value[name_end]) || value[name_end] == '_')) {
            name_end++;
          }
          if (name_end > pos + 2) {
            auto name = value.substr(pos + 2, name_end - pos - 2);
            auto next = value.find("/:" + name, name_end);
            if (next != std::string::npos) {
              return false;
            }
          }
          pos = value.find("/:", name_end);
        }
      }

      // Skip hostname patterns with invalid characters (not yet validated)
      ...
      // Skip patterns with emojis (require full Unicode ID validation)
      for (std::size_t i = 0; i < value.size(); i++) {
        const auto byte = static_cast<unsigned char>(value[i]);
        // Check for 4-byte UTF-8 sequences (emoji range typically starts there)
        if (byte >= 0xF0) {
          return false;
        }
      }
...
public:
  ...
  void TestBody() override {
    // Error test: input_json is null means we expect pattern parsing to fail
    if (this->input_json.is_null()) {
      const auto maybe_pattern{
          sourcemeta::core::URLPattern::parse(this->pattern_json)};
      EXPECT_FALSE(maybe_pattern.has_value())
          << "Expected pattern parsing to fail for error test";
      return;
    }
...
int main(int argc, char **argv) {
  ...
  for (const auto &test_case : match_test_data.as_array()) {
    ...
    // For error tests, use null for input and expected_match
    if (is_error_test(test_case)) {
      const auto null_json = sourcemeta::core::JSON{nullptr};
      testing::RegisterTest(
          "URLPattern_wpt_match", test_name.c_str(), nullptr, nullptr, __FILE__,
          __LINE__, [=]() -> URLPatternMatchTest * {
            return new URLPatternMatchTest(pattern, null_json, null_json);
          });
    } else {
