...
TEST(JSONSchema_frame_draft6, location_independent_identifier_anonymous) {
  ...
  EXPECT_FRAME_LOCATION_NON_REACHABLE(frame, Static, "#/definitions/bar",
                                      "#/definitions/foo");
  EXPECT_FRAME_LOCATION_REACHABLE(frame, Static, "#foo", "#/definitions/foo");
...
TEST(JSONSchema_frame_draft6, propertyNames_with_nested_applicators) {
  ...
  EXPECT_FRAME_LOCATION_NON_REACHABLE(
      frame, Static, "#/propertyNames/definitions/test", frame.root());
}

TEST(JSONSchema_frame_draft6, top_level_id_empty_fragment_only) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
    "$id": "#",
    "$schema": "http://json-schema.org/draft-06/schema#"
  })JSON");
...
TEST(JSONSchema_frame_draft6, top_level_id_empty_string) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
    "$id": "",
    "$schema": "http://json-schema.org/draft-06/schema#"
  })JSON");
