...
TEST(JSONSchema_official_walker_draft4, definitions) {
  using namespace sourcemeta::core;
  const auto &result{
      schema_official_walker("definitions", VOCABULARIES_DRAFT4)};
...
TEST(JSONSchema_official_walker_draft4, uniqueItems) {
  using namespace sourcemeta::core;
  const auto &result{
      schema_official_walker("uniqueItems", VOCABULARIES_DRAFT4)};
  EXPECT_EQ(result.type, SchemaKeywordType::Assertion);
  EXPECT_TRUE(result.vocabulary.has_value());
  EXPECT_VOCABULARY_KNOWN(result.vocabulary.value(), JSON_Schema_Draft_4);
  const std::unordered_set<std::string_view> expected{"$ref"};
...
TEST(JSONSchema_official_walker_draft4, description) {
  using namespace sourcemeta::core;
  const auto &result{
      schema_official_walker("description", VOCABULARIES_DRAFT4)};
  EXPECT_EQ(result.type, SchemaKeywordType::Comment);
  EXPECT_TRUE(result.vocabulary.has_value());
  EXPECT_VOCABULARY_KNOWN(result.vocabulary.value(), JSON_Schema_Draft_4);
  const std::unordered_set<std::string_view> expected{"$ref"};
