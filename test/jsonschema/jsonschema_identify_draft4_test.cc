...
TEST(JSONSchema_identify_draft4, base_dialect_shortcut) {
  ...
  const auto id{sourcemeta::core::identify(
      document, sourcemeta::core::SchemaBaseDialect::JSON_Schema_Draft_4)};
  EXPECT_EQ(id, "https://example.com/my-schema");
}
...
TEST(JSONSchema_identify_draft4, id_empty_fragment_only) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
    "id": "#",
    "$schema": "http://json-schema.org/draft-04/schema#"
  })JSON");
...
TEST(JSONSchema_identify_draft4, id_empty_string) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
    "id": "",
    "$schema": "http://json-schema.org/draft-04/schema#"
  })JSON");
