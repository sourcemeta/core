...
TEST(JSONSchema_vocabulary_draft1, schema) {
  ...
  EXPECT_VOCABULARY_REQUIRED(vocabularies, JSON_Schema_Draft_1);
}

TEST(JSONSchema_vocabulary_draft1, hyperschema) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
  ...
  const sourcemeta::core::Vocabularies vocabularies{
      sourcemeta::core::vocabularies(document, test_resolver)};
  EXPECT_EQ(vocabularies.size(), 1);
  EXPECT_VOCABULARY_REQUIRED(vocabularies, JSON_Schema_Draft_1_Hyper);
}

TEST(JSONSchema_vocabulary_draft1, one_hop) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
...
TEST(JSONSchema_vocabulary_draft1, ignore_vocabulary) {
  const sourcemeta::core::JSON document = sourcemeta::core::parse_json(R"JSON({
