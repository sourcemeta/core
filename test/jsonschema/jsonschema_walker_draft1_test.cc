...
TEST(JSONSchema_walker_draft1, instance_locations) {
  ...
  // Applicators (object)
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 1, "/properties/foo", "", "/foo", "/foo");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 2, "/properties/foo/requires",
                             "/properties/foo", "", "");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 3, "/properties/bar", "", "/bar", "/bar");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 4, "/additionalProperties", "",
                             "/~?additionalProperties~/~P~",
                             "/~?additionalProperties~/~P~");

  // Applicators (array)
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 5, "/items", "", "/~I~", "/~I~");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 6, "/items/items/0", "/items", "/~I~/0",
                             "/0");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 7, "/items/items/1", "/items", "/~I~/1",
                             "/1");

  // Applicators (any)
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 8, "/type/1", "", "/~?type~/~?1~",
                             "/~?type~/~?1~");
  EXPECT_WALKER_ENTRY_DRAFT1(entries, 9, "/type/2", "", "/~?type~/~?2~",
                             "/~?type~/~?2~");
