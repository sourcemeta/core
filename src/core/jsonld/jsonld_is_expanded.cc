...
// A list or set object "MUST NOT contain any other entries" beyond its defining
// keyword and an optional @index (JSON-LD 1.1 Section 9, "list object" and "set
// object").
auto is_list_or_set_object(const JSON &value, const JSON::StringView keyword,
                           const JSON::Object::hash_type keyword_hash) -> bool {
  for (const auto &entry : value.as_object()) {
    if (entry.key_equals(keyword, keyword_hash)) {
      if (!is_property_value(entry.second)) {
        return false;
      }
  ...
  return value.defines(keyword, keyword_hash);
}
...
// @language" (JSON-LD 1.1 Section 9, "value object").
auto is_value_object(const JSON &value) -> bool {
  for (const auto &entry : value.as_object()) {
...
// "A node object MUST NOT contain the @value, @list, or @set keywords"
// (JSON-LD 1.1 Section 9, "node object"). @id and @type carry IRI references,
// which may be relative, whereas property terms are absolute IRIs or blank node
// identifiers.
auto is_node_object(const JSON &value) -> bool {
  if (!value.is_object()) {
    return false;
  ...
  for (const auto &entry : value.as_object()) {
    ...
    } else if (!is_term(key) || !is_property_value(entry.second)) {
      return false;
    }
  }
...
auto is_property_item(const JSON &value) -> bool {
  ...
  if (value.defines(KEYWORD_LIST, KEYWORD_LIST_HASH)) {
    return is_list_or_set_object(value, KEYWORD_LIST, KEYWORD_LIST_HASH);
  }
  if (value.defines(KEYWORD_SET, KEYWORD_SET_HASH)) {
    return is_list_or_set_object(value, KEYWORD_SET, KEYWORD_SET_HASH);
  }
  return is_node_object(value);
}
