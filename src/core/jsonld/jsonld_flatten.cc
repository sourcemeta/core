...
// The identifiers of a graph's nodes in lexicographical order.
auto sorted_ids(const JSON &graph) -> std::vector<JSON::String> {
  std::vector<JSON::String> ids;
...
auto flatten(BlankNodeState &state, const JSON &element) -> JSON {
  ...
  node_map.assign_assume_new(JSON::String{KEYWORD_DEFAULT},
                             JSON::make_object());
  ...
  std::vector<JSON::String> graph_names;
  ...
  auto &default_graph{node_map.at(KEYWORD_DEFAULT)};
