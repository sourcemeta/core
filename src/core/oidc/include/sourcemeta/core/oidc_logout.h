...
/// `iat` must be present and not in the future, an `exp` when present must not
/// have passed, the `typ` header must be `logout+jwt`, the token must carry a
/// `sub` or `sid`, an `events` object with the back-channel logout member, a
/// `jti`, and it must not carry a `nonce`. Replay rejection by `jti` is the
/// caller's responsibility. For example:
///
/// ```cpp
/// #include <sourcemeta/core/oidc.h>
/// #include <array>
