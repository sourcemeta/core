...
// OpenAPI Specification 3.1.1, Section 4.10: "The OpenAPI Schema Object
// dialect for this version of the specification is identified by the URI
// `https://spec.openapis.org/oas/3.1/dialect/base`"
constexpr auto OPENAPI_DIALECT{
...
constexpr std::array<JSON::StringView, 10> OPENAPI_ROOT_FIELDS{
...
// OpenAPI Specification 3.1.1, Section 3 lists five ways an implementation MAY
// tell what a referenced document is, and this takes the second of them,
// "Detecting OpenAPI documents through the root `openapi` field". A document
// without one may be a bare holder of referenceable Objects or a Schema
// Object, and 3.1 gives no ground to turn either down
...
inline auto openapi_follow_internal_reference(const URI &target,
  ...
  // Section 4.8.11 requires a `$ref` to "be in the form of a URI" and says
...
inline auto openapi_follow_reference(const JSON::StringView reference,
  ...
  try {
    target.emplace(JSON::String{reference});
    if (!walk.base.empty()) {
      target.value().resolve_from(URI{walk.base});
    }
  ...
  const auto identifier{target.value().recompose_without_fragment()};
  const auto names_a_fragment{target.value().fragment().has_value()};
  ...
  // Section 3 requires complete documents to be parsed rather than the
...
inline auto openapi_check_document(const JSON &document, OpenAPIWalk &walk)
  ...
  try {
    ...
    if (!openapi_version(document).has_value()) {
      throw OpenAPIError{Pointer{"openapi"},
    ...
    openapi_reject_unknown_fields(
        document, OPENAPI_ROOT_FIELDS, EMPTY_POINTER,
        "The OpenAPI Object does not define this field");
    ...
    // Object". Section 3 leaves which document's Components Object that is to
    // the implementation and says "it is RECOMMENDED that tools resolve from
    // the entry document, rather than the current document", so the names come
    // from there. They are taken before anything else is read, as a document
    // a reference brings in may declare a requirement long before the entry
    // document's own components have been walked
    // Section 4.8.10 has an Operation Object's tags name Tag Objects "found
    // under the root OpenAPI Object", and Section 3 recommends the entry
    // document for the same reason it does for security schemes, so both sets
    // of names come from there and both come before anything else is read
    if (walk.entry) {
      openapi_collect_security_schemes(document, walk);
      openapi_collect_tags(document, walk);
    }

    // Section 3: an OpenAPI Description "MUST contain at least one paths
    ...
    // Section 4.10: "To allow use of a different default `$schema` value for
    // all Schema Objects contained within an OAS document, a
    ...
    JSON::String effective_dialect{OPENAPI_DIALECT};
    if (dialect != nullptr) {
    ...
    if (security != nullptr) {
      ...
      // top-level security", and Section 3 makes the entry document the one
