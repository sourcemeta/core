#ifndef SOURCEMETA_CORE_JSONPATH_EVALUATOR_H_
#define SOURCEMETA_CORE_JSONPATH_EVALUATOR_H_
...
#include <variant> // std::get, std::holds_alternative, std::monostate, std::variant
...
auto filter_matches(const FilterExpression &expression, const JSON &candidate,
                    const JSON &root) -> bool;
...
inline auto slice_bounds(const SelectorSlice &slice, const std::int64_t size,
                         std::int64_t &lower, std::int64_t &upper) -> bool {
  if (slice.step == 0) {
    return false;
  }
...
auto filter_query_visit(const std::vector<JSONPathSegment> &segments,
  ...
  for (const auto &selector : segment.selectors) {
    switch (static_cast<SelectorKind>(selector.index())) {
      case SelectorKind::Name: {
        const auto *entry{std::get_if<SelectorName>(&selector)};
      ...
      case SelectorKind::Index: {
        const auto *entry{std::get_if<SelectorIndex>(&selector)};
        if (current.is_array()) {
      ...
      case SelectorKind::Slice: {
        const auto *entry{std::get_if<SelectorSlice>(&selector)};
        if (current.is_array()) {
          const auto size{static_cast<std::int64_t>(current.as_array().size())};
          std::int64_t lower{0};
          std::int64_t upper{0};
      ...
      case SelectorKind::Filter: {
        const auto *entry{std::get_if<SelectorFilter>(&selector)};
        if (current.is_object()) {
          for (const auto &member : current.as_object()) {
...
inline auto resolve_singular(const FilterQuery &query, const JSON &candidate,
                             const JSON &root) -> const JSON * {
  ...
  for (const auto &segment : query.segments) {
    ...
    if (std::holds_alternative<SelectorName>(selector)) {
      const auto *entry{std::get_if<SelectorName>(&selector)};
...
auto evaluate_value_function(const FilterFunctionCall &call,
...
inline auto evaluate_logical_function(const FilterFunctionCall &call,
                                      const JSON &candidate, const JSON &root)
    -> bool {
  ...
  const auto compiled{to_regex(expression->to_string(),
...
inline auto filter_compare(const FilterComparison &comparison,
  ...
  switch (comparison.operation) {
    ...
    case FilterComparisonOperator::NotEqual:
      return !filter_equals(left_value, right_value);
    case FilterComparisonOperator::Less:
      return filter_less(left_value, right_value);
    case FilterComparisonOperator::LessEqual:
      return filter_less(left_value, right_value) ||
             filter_equals(left_value, right_value);
    case FilterComparisonOperator::Greater:
      return filter_less(right_value, left_value);
    case FilterComparisonOperator::GreaterEqual:
      return filter_less(right_value, left_value) ||
             filter_equals(left_value, right_value);
  }
...
inline auto filter_matches(const FilterExpression &expression,
                           const JSON &candidate, const JSON &root) -> bool {
  if (std::holds_alternative<FilterComparison>(expression.value)) {
    return filter_compare(std::get<FilterComparison>(expression.value),
                          candidate, root);
  }

  if (std::holds_alternative<FilterTest>(expression.value)) {
    const auto &test{std::get<FilterTest>(expression.value)};
    bool result{false};
    if (std::holds_alternative<FilterQuery>(test.subject)) {
      const auto &query{std::get<FilterQuery>(test.subject)};
  ...
  if (std::holds_alternative<FilterConjunction>(expression.value)) {
    const auto &conjunction{std::get<FilterConjunction>(expression.value)};
  ...
  if (std::holds_alternative<FilterDisjunction>(expression.value)) {
    const auto &disjunction{std::get<FilterDisjunction>(expression.value)};
...
inline auto evaluate_selectors(const std::vector<JSONPathSegment> &segments,
                               const std::size_t cursor, const JSON &current,
                               EvaluationState &state) -> void {
...
// each visited node
inline auto evaluate_descendant(const std::vector<JSONPathSegment> &segments,
                                const std::size_t cursor, const JSON &current,
                                EvaluationState &state) -> void {
...
inline auto evaluate_segments(const std::vector<JSONPathSegment> &segments,
                              const std::size_t cursor, const JSON &current,
                              EvaluationState &state) -> void {
  if (cursor == segments.size()) {
