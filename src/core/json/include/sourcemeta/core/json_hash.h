...
template <typename T> struct PropertyHashJSON {
  ...
  [[nodiscard]]
  inline auto perfect(const std::string_view value,
  ...
  template <typename U>
    requires requires(const U &wrapper) { wrapper.get(); }
  inline auto operator()(const U &value) const noexcept -> hash_type {
    return (*this)(std::string_view{value.get()});
  }

  inline auto operator()(const std::string_view value) const noexcept
      -> hash_type {
