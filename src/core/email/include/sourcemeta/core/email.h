...
/// @ingroup email
/// Produce the domain half of a valid RFC 5321 `Mailbox`, with no result when
/// the input is not one. The separator is located by parsing the address,
/// since neither the first nor the last at sign reliably marks it: RFC 5321
/// Section 4.1.2 admits one inside a quoted local part, and Section 4.1.3
/// admits one inside the content of a General-address-literal. The result
/// borrows from the input and keeps its spelling, so a caller comparing
...
/// #include <cassert>
///
/// const auto domain{sourcemeta::core::email_domain("\"not@me\"@example.org")};
/// assert(domain.has_value());
/// assert(domain.value() == "example.org");
/// ```
SOURCEMETA_CORE_EMAIL_EXPORT
