...
auto get(const JSON &document [[clang::lifetimebound]], const Pointer &pointer) -> const JSON & {
  if (pointer.empty()) {
    return document;
  }
...
auto get(const JSON &document [[clang::lifetimebound]], const WeakPointer &pointer) -> const JSON & {
  if (pointer.empty()) {
    return document;
  }
